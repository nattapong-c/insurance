...
export const deleteInvoiceAction = (data) => async (dispatch) => {
    if (data?.clear) {
        dispatch({ type: TYPE.INVOICE_DELETE_CLEAR });
        return;
    }
    dispatch({ type: TYPE.INVOICE_DELETE_REQ });
    try {
        const response = await API.deleteInvoiceList(data);
        dispatch({ type: TYPE.INVOICE_DELETE_SUCCESS, payload: response.data });
    } catch (err) {
        dispatch({ type: TYPE.INVOICE_DELETE_FAIL, payload: err.response.data.error });
    }
};
